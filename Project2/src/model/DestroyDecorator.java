...
public class DestroyDecorator extends TetrisPieceDecorator{
    ...
    private int width;
    private int height;
    public DestroyDecorator next; // We'll use this to link each piece to its "next" rotation.
    static public DestroyDecorator[] pieces;	// array of rotations for this piece
    ...
    public DestroyDecorator(TetrisPoint[] points, TetrisModel tm){
        super(points, tm);
        body = points;
        int ymax = body[0].y;
        int ymin = body[0].y;
    ...
    @Override
    public void action() {
        tm.score += 3 *  tm.board.getMaxHeight();
        for(int i = 0; i < tm.board.getMaxHeight(); i++){
            for(int j =0; j < tm.board.getWidth(); j++){
                if(tm.board.getGrid(j,i)){
                    tm.board.tetrisGrid[j][i] = false;
                }
            }
        }
     ...
     */
    public static DestroyDecorator[] getPieces(TetrisModel tm1) {
        tm = tm1;
        // lazy evaluation -- create static array only if needed
        if (DestroyDecorator.pieces==null) {
            // use makeFastRotations() to compute all the rotations for each piece
            try {
                DestroyDecorator.pieces = new DestroyDecorator[]{
                        makeFastRotations(new DestroyDecorator(parsePoints(SQUARE_STR), tm)),
                };
        ...
        return DestroyDecorator.pieces;
    }
     ...
     */
    public static DestroyDecorator makeFastRotations(DestroyDecorator root) {
     ...
     * @return the next rotation of the given piece
     */
    public DestroyDecorator computeNextRotation() {
        ...
        return new DestroyDecorator(tps, tm);
    }
    /**
